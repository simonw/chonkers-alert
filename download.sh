...
if [ "$EXTENSION" = ".json" ]; then
  ...
  if command -v jq &> /dev/null; then
    if jq '.results = (.results | map({id, time_observed_at, observed_on_details, observed_time_zone, observation_photos, geojson, place_guess, user: (.user | {login, name, icon_url}), uri, description}))' "$TEMP_FILE" > "$PRETTY_TEMP" 2>/dev/null; then
